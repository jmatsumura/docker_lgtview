...
docker exec -it dockerlgtview_LGTview_1 openssl req -x509 -nodes -days 1460 -newkey rsa:2048 \
	...
	-subj "/C=$country/ST=$statei/L=$city/O=$organization/OU=$division/CA=TRUE/CN=lgtview"
...
docker exec -it dockerlgtview_LGTview_1 sed -i "3a\\\t\tServerName lgtview" /etc/apache2/sites-available/default-ssl.conf
docker exec -it dockerlgtview_LGTview_1 sed -i "4a\\\t\tServerAlias twinblast" /etc/apache2/sites-available/default-ssl.conf
